...
echo "Waiting for Potabi media to initialize"
while : ; do
    [ -e "/dev/iso9660/Potabi" ] && echo "found /dev/iso9660/Potabi" && break
    sleep 1
done

echo "==> Mount cdrom"
mount_cd9660 /dev/iso9660/Potabi /cdrom
mdconfig -f /cdrom/data/system.uzip -u 1
zpool import potabi -o readonly=on
...
if [ $x -gt $y ] ; then 
    echo "Potabi requires 4GB of memory for memdisk, and operation!"
